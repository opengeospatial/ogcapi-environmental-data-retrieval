...
```
There are also other variants of these query patterns to make the API easier and more convenient to use. 
...
*	EDR API queries are “fixed” and predefined in the OpenAPI definition;
...
*	The Feature Sampling Geometry type allows users to get exactly the data that they need;
*	Users do not need to know about the structure of the underlying data because the EDR-API abstracts that detail from the user;
*	It is not constrained to a particular data structure. The EDR can handle grids, features, etc.;
*	It hides the complication of any underlying time structure. Queries are simply an extraction of the data for the time the user selects;
	It is the responsibility of the publisher to simplify the output, making it very convenient for the user to consume the data;
