...
include::../recomendations/core/REC_rc-next-1.adoc[]

include::../recomendations/core/REC_rc-next-2.adoc[]

include::../recomendations/core/REC_rc-next-3.adoc[]
...
Another implementation approach is to use an implementation-specific parameter that specifies the index within the result set from which the server begins presenting results in the response, like the `startIndex` parameter that was used in <<WFS20,WFS 2.0>> (and which could be added again in additional parts of the OGC API Features series).
...
include::../recomendations/core/PER_rc-prev.adoc[]
