...
The primary messaging mechanism used for implementation of the EDR API Standard is JSON, including CoverageJSON, over HTTP(S). Implementations of the EDR API Standard are described using the OpenAPI V3.0 or V3.1 specifications. The target users are web-developers and end-users who are not geospatial experts. The target data resources are any data described as spatiotemporal, accessible by coordinates, not just meteorological or oceanographic.
