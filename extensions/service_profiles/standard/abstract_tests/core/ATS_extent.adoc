...
test-purpose:: Validate the service profile extent requirements are defined correctly. 
test-method:: 
step:: Verify that the service profile has a requirement specifying the spatial extent of the `Collection`.
step:: Verify that the service profile specifies either an enumerated list of CRS values or a Regular expression definition for valid CRS values.
