...
The Profile is conformant with the ModSpec

include::../requirements/core/REQ_modspec.adoc[]

Implementations of the Profile are conformant with OGC API-EDR Part 1: Core
...
A resource constrained by a Service Profile SHOULD include a link with a link relation type of `profile` that references the Service Profile defining the rules implemented by the response.

NOTE: Resources defined by Service Profiles can include /root, /collections, and /instances.
...
The landing page provides links that support exploration of the resources offered via the API. The most important component of a landing page is a list of links. The Landing Page resource is initially defined in the Core conformance class of the <<OGC19-072,OGC API — Common - Part 1>> Standard. The <<ogc-edr,OGC API - Environmental Data Retrieval Standard>> does not make any changes to this definition. The normative JSON Schema for an EDR Landing Page is defined in the https://schemas.opengis.net/ogcapi/edr/1.2/openapi/schemas/core/landingPage.yaml[LandingPage.yaml] document.

Profiles of the <<ogc-edr,OGC API – Environmental Data Retrieval Standard>> are able to provide a richer description of the API by mandating optional attributes. The requirements below define additional content rules that service profiles can enforce.
...
Each service profiles of the <<ogc-edr, OGC API – Environmental Data Retrieval>> standard must provide an OpenAPI 3.1 document when the profile includes requirements that can be expressed using the OpenAPI specification. A service profile document is a subset of the API definition produced by the OGC API-EDR implementation and reflects the additional requirements specified by the profile.
...
include::../requirements/core/REQ_conformance-classes.adoc[]
...
NOTE: Profile developers should get guidance from the https://docs.ogc.org/pol/10-103r1.html[OGC Naming Authority] on valid URIs for Profiles. 
...
An implementation of the OGC API-EDR may support multiple collections. Additional requirements address how the Service Profile should document requirements at the per-collection level as well as on the landing page (where appropriate)
...
While Web protocols typically use request-response operations, there is also support for asychronous operations.
...
==== EDR Part 2 Publish Subscribe Workflows
