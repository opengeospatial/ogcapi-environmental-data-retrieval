...
Service profiles of the <<ogc-edr, OGC API – Environmental Data Retrieval>> standard must provide an OpenAPI 3.1 document when the profile includes requirements that can be expressed using the OpenAPI specification. This service profile document is a subset of the API definition produced by the OGC API‑EDR implementation and reflects the additional requirements specified by the profile.
