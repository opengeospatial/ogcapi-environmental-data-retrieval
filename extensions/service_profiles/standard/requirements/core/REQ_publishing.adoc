...
statement:: An EDR service profile _SHALL_ be published as an OpenAPI JSON document when it contains requirements that can be encoded in the OpenAPI specification. 
part:: The OpenAPI compatible rules described in the requirements _SHALL_ be encoded using the OpenAPI 3.1 specification.
part:: The OpenAPI compatible requirement rules _SHALL_ be encoded in either the OpenAPI Path Item or in the Response object schema sections of the document.
...
statement:: A profile OpenAPI document _SHALL_ describe the profile EDR API as follows:
...
part:: data_query requirement rules _SHALL_ be encoded in the JSON schema defined in the 200 responses for the /collections and /collections/{collectionId} Paths object
