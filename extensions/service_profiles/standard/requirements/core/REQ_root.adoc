...
statement:: A Profile MAY impose requirements on the complete OGC API service implementation or on a defined subset of the functionality it provides.
...
====
[%metadata]
identifier:: /rec/core/root-service-profile
statement:: 

part:: The root SHALL clearly indicate when service profile defines rules that apply to its contents.

part:: The root SHOULD provide a link to its service profile definition with a link relation type of `profile`.
...
statement:: While the OGC API-EDR Landing Page schema requires only the links property, a Profile of the <<ogc-edr,OGC API - Environmental Data Retrieval Standard>> _SHALL_ require the following additional properties and content:
...
statement:: While the OGC API-EDR Landing Page schema requires only the links property, a Profile of the <<ogc-edr,OGC API – Environmental Data Retrieval Standard>> _SHOULD_ mandate the following additional properties.
