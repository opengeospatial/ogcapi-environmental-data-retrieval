[[ats_oas3]]
[cols="1,4",width="90%"]
.Conformance Class "OpenAPI 3.0"
|===
2+|*Conformance Class*
2+|http://www.opengis.net/spec/ogcapi-edr-1/1.1/conf/oas30
|Target type |Web API
|Requirements Class |<<rc_oas30,http://www.opengis.net/spec/ogcapi-edr-1/1.1/req/oas30>>
|Dependency |<<ats_core,http://www.opengis.net/spec/ogcapi-edr-1/1.1/conf/core>>
|Dependency |http://www.opengis.net/spec/ogcapi-common-1/1.0/conf/oas30[http://www.opengis.net/spec/ogcapi-common-1/1.0/conf/oas30]
|===

include::oas30/ATS_completeness.adoc[]

include::oas30/ATS_exception-codes.adoc[]

include::oas30/ATS_definition-1.adoc[]

include::oas30/ATS_definition-2.adoc[]

include::oas30/ATS_oas-impl.adoc[]

include::oas30/ATS_security.adoc[]
