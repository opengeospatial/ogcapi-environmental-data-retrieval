...
^|Requirement |<<req_edr_corridor-width-response,/req/edr/REQ_rc-corridor-width-response>>
^|Test Method |. Verify that a `400` error will be generated if corridor-width is not specified
. Validate that the `corridor-width` parameter complies with the syntax described in <<req_edr_corridor-width-response,/req/edr/REQ_rc-corridor-width-response>>.
|===
