...
step::: If the `variables` object has a `crs` property verify that it is an array of crs objects, and each member object of the array has `crs_details` and `wkt` properties.
====