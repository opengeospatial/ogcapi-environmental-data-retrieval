...
|===
^|*Abstract Test {ats-id}* |*/conf/area*
^|Test Purpose |Validate that an error is returned by a `Area` query when the coords query parameter does not contain a valid POLYGON or MULTIPOLYGON Well Known Text value.
^|Requirement |<<_req_edr_rc-area,/req/edr/rc-area>>
