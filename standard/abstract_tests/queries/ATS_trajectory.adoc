...
^|Requirement |<<_req_edr_rc-trajectory,/req/edr/rc-trajectory>>
^|Test Method |. Check coords query parameter with time parameter is a valid Well Known Text LINESTRINGZM or MULTILINESTRINGZM value, the `Z` coordinate shall be a within the range of vertical levels advertised in the Collection metadata
. Validate that a document was returned with a status code 400.
|===
{counter2:ats-id}
[width="90%",cols="2,6a"]
