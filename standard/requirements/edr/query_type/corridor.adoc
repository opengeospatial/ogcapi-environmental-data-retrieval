The Corridor query returns data along and around the path defined by the `coords` parameter.  *Logic for identifying the best match for the requested corridor will depend on the <<collection-definition,collection>> and is at the discretion of the query implementer* .  The results are further filtered by the constraints defined by the following query parameters:
