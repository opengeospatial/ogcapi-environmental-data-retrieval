The Trajectory query returns data along the path defined by the `coords` parameter. *Logic for identifying the best matches for the requested trajectory will depend on the <<collection-definition,collection>> and is at the discretion of the query implementer* .  The results are further filtered by the constraints defined by the following query parameters:
...
* A 3D corridor, each waypoint at a different time by at the same height:
...
* A 3D corridor, each waypoint at a different height:
`coords=LINESTRINGZ(-3.53 50.72 0.1,-3.35 50.92 0.2,-3.11 51.02 0.3,-2.85 51.42 0.4,-2.59 51.46 0.5)`

* A 3D corridor each waypoint at a different height but the same time:
...
* A 4D corridor, each waypoint at different heights and times:
