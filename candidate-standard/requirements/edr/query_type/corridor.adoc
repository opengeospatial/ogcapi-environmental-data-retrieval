The Corridor query returns data along the path defined by the `coords` parameter, *the logic to match the data for the requested path will depend on and be defined by the Collection*.  The results are further filtered by the constraints defined by the following query parameters:
