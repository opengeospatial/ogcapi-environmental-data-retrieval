...
`{instanceId}` = a text string identifying the version or instance of the chosen <<collection-definition,collection>>.
...
Information resources associated with a specific <<collection-definition,collection>> should be accessed through the `/collections` path. Information resources not associated with a specific collection should be accessed via the `/{instanceId}/{queryType}` path template.
...
If not specified, the query will return data in the native format of the <<collection-definition,collection>>.  If the requested format system does not match an entry in the defined list of valid `outputFormats` for the <<collection-definition,collection>>, a `400` message SHOULD be returned.
