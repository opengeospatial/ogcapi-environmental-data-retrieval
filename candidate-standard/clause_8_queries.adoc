...
=== Query and Environmental Resources
:sectnums:

Query resources are spatial queries which support the operation of the API or the access and use of the Environmental Data resources. The API-EDR standard has identified an initial set of common `queryTypes` to implement. This list may change as the standard is used. The `queryTypes` are described in the <<query-resources>> section.
