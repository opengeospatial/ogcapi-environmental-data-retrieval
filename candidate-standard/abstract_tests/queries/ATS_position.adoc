...
^|*Abstract Test {counter:ats-id}* |*/ats/collections/rc-op*
^|Test Purpose |Validate that an error is returned by a `Position` query when at least one of the required query parameters are not specified.
^|Requirement |<<req_collections_rc-op,/req/collections/rc-op>>
...
and verify that only features are returned that match the selection criteria.
|===
