...
^|Requirement |<<req_edr_rc-max-z-response,/req/edr/max-z-response>>
...
. Validate that the `max-z` vertical level parameter complies with the syntax described in <<req_edr_rc-max-z-response,/req/edr/max-z-response>>.
|===
