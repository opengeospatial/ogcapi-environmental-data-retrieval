...
The goals of the EDR API are to make it easier to access a wide range of data through a uniform, well-defined Web interface, and to achieve data reduction to just the data needed by the user or client. A major use case for the EDR API is to retrieve small subsets from large collections of of environmental data, such as weather forecasts, though many other types of data can be accessed. The important aspect is that the data can be unambiguously specified by spatio-temporal coordinates.  
