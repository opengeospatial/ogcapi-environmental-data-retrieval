...
A major use case for the EDR API is to retrieve small subsets from large collections of of environmental data, such as weather forecasts, though many other types of data can be accessed. The important aspect is that the data can be unambiguously specified by spatio-temporal coordinates.  
